...
namespace estate_executor {
    int assemble_account_number(int secret_modifier){
        int secret_number = zhang::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier) + garcia::bank_number_part(secret_modifier);
        return secret_number;
    }
    int assemble_code(){
        int secret_code = (zhang::blue::code_fragment() + khan::blue::code_fragment() + garcia::blue::code_fragment()) * (khan::red::code_fragment() + garcia::red::code_fragment() + zhang::red::code_fragment());
        return secret_code;
    }
}